...
def is_valid_guess(guess, game_data):
    ...
    """
    msg = "" # String that will store the message to print
    if not str.isalpha(guess):
        game_data["warnings"] -= 1
        msg += "Oops! That is not a valid letter. "
    ...
    if game_data["warnings"] >= 0:
        msg += f"You have {game_data['warnings']} warnings: {game_data['guessed_word']}"
    else:
        game_data["guesses"] -= 1
        game_data["warnings"] = 3
        msg += f"You have no warnings left so you lose one guess: {game_data['guessed_word']}"
    print(msg)
    return False
...
def make_guess(guess, game_data):
    ...
    if guess in game_data["secret_word"]:
        game_data["guessed_word"] = get_guessed_word(
            game_data["secret_word"], game_data["letters_guessed"]
        )
...
def hangman(secret_word):
    ...
    * After each guess, you should display to the user the
    ...
    print(f"You have {game_data['warnings']} warnings left.")
